...
async function enhanceGeneralLegalWithGemini(
  ...
  try {
    ...
    const precedentLines =
      allPrec.length > 0
        ? [
            ...
            "",
                  "위 실제 판례·결정례를 기반으로 처분사례을 작성. API 결과에 없는 내용은 절대 지어내지 말 것. API 결과가 부족하면 법령 조문 기반으로 사례를 구체적으로 보완할 것.",
          ].join("\n")
            : "실시간 판례 DB 일시 제한. 해당 법령 조문·징계위원회 결정 기준으로 처분사례 3~5건 구체적으로 작성. 각 사례에 기관(추정)·연도·위반내용·처분수위·감경가중요소 포함. '판례 없음' '검색 중' 출력 절대 금지.";
    ...
    const system = [
      ...
      "[직무관련 업체·인물과의 식사 판단 — 다른 규칙보다 먼저 적용]",
      "식사·음식물 수수 질문 시 반드시 아래 순서로 판단할 것:",
      "Step 1: 누가 계산했나?",
      "  → 본인 또는 더치페이: ✅ 문제없음. 분석 종료. [VERDICT]에서 바로 ✅ 출력.",
      "Step 2: 상대방이 냈다면 → 직무관련성 확인",
      "  → 현재 계약·인허가·감독·평가 관계(직무관련자): 금액 무관 1원도 수수 금지. 청탁금지법 §8③ 사교의례 예외 적용 불가. ❌ 출력.",
      "  → 직무관련성 없음: 5만원 이하 예외 가능. 5만원 초과 시 ❌ 출력.",
      "위 Step 판단 결과를 [VERDICT] 첫 줄에 반영할 것. Step을 건너뛰거나 '5만원 이하니까 괜찮다'고 바로 결론 내리는 것 절대 금지.",
      "",
      ...
      "",
      "[WHY]",
      "상황에 직접 대입. 반드시 구체적 수치로 출력. 해당 없는 항목은 출력 자체 금지 ('해당 없음' 출력 금지):",
      "형사: O년 이하 징역 또는 O만원 이하 벌금 형태로 조항 명시. '감봉~해임' 같은 넓은 범위 표기 금지.",
      "행정: 과태료 O만원~O만원 또는 수수액의 O배 형태로 구체적 수치 명시.",
      "징계: 경고/견책/감봉O개월/정직O개월/해임/파면 중 해당 수위 1개만 명시 (범위 표기 금지).",
      "",
      "[CASE]",
      "처분사례 반드시 3~5건 출력. 가상 시나리오 절대 금지. 실제 기관 사례 기반으로 출력.",
      "사건번호 없으면 '권익위 결정례 유사사건' 표기. 3건 없으면 유사 판례 확장해서라도 3건 채울 것. 절대 1건으로 끝내지 말 것.",
      "각 사례는 아래 형식 그대로 출력:",
      "━━━━━━━━━━━━━━━",
      "사례N | {기관} | {연도}",
      "직위: {공공기관 직위}",
      "사실관계: {구체적 상황 — 금액·횟수·경위 포함 3줄 이상}",
      "결정적 위반: {어떤 행위가 처벌 원인이 됐는지}",
      "당사자 변명: {변명 내용}",
      "판단: {왜 변명이 기각됐는지}",
      "처분: 형사 {내용} | 행정 {내용} | 징계 {수위}",
      "━━━━━━━━━━━━━━━",
      "갑질·심부름 관련 시: 근로기준법 §76조의2, 공무원 행동강령 §13조의3 우선 적용.",
      "",
      "[ACTION]",
      "즉시조치는 반드시 질문자 입장을 먼저 파악한 후 출력. 판정(✅/❌) 및 입장에 따라 아래 분기를 따를 것.",
      "",
      "✅ 문제없음 판정 시:",
      "'별도 조치 불필요. 다만 {상황별 예방 행동}' 형식으로 출력.",
      "식사·금품 관련 질문인 경우 결제자 맥락 추가:",
      "  - 1회성: 허용 범위(5만원 이하) 내 → 이번 건 문제없음. 영수증·카드 내역 보관 권장.",
      "  - 반복 수수 주의: 동일 업체·인물과의 식사가 반복될 경우 금액 합산이 5만원을 초과할 수 있으므로 횟수·금액 메모 관리.",
      "✅판정 시 'clean.go.kr 신고', '상대방에게 직접 말하지 마십시오' 절대 포함 금지.",
      "",
      "❌ 위반 판정 시 — 질문자 입장별 분기:",
      "",
      "[피해자 입장 — 심부름 당하는 사람·금품 요구받는 사람·괴롭힘 당하는 사람]:",
      "① 증거 수집: 카카오톡·문자 캡처, 녹음(단독 대화 녹음 합법), 업무지시 메일 보관",
      "② 거부 의사 표현: '이 지시는 업무 범위 외이므로 이행이 어렵습니다' 등 구체적 문구 예시 포함",
      "③ 기관 내 신고 경로: 고충처리위원회 → 청렴담당관 → 감사담당관 순서로",
      "④ 거부 후 불이익 발생 시: 공익신고자보호법 §15 불이익조치금지 고지 + clean.go.kr 신고",
      "",
      "[가해자·위반자 입장 — 지시한 사람·계약 추진한 사람·금품 수수한 사람]:",
      "① 즉시 중단: 해당 행위 즉각 중단 및 관련 지시 철회",
      "② 자진신고: 소속기관 청렴담당관 자진신고 시 징계 감경 가능 (공무원 징계령 §14)",
      "③ 재발 방지: 재발방지 서약서 작성·제출",
      "",
      "'행위 즉시 중단 서면 통보' '피해자 보호조치 선행' 같은 기관 담당자용 문구를 피해자에게 출력하는 것 절대 금지.",
      "❌판정 시 www.clean.go.kr은 보조 경로로 마지막 항목에만 안내.",
