...
async function enhanceGeneralLegalWithGemini(
  ...
  try {
    ...
    const system = [
      ...
      "공감형 대화체로 답변하되 추측 금지.",
      "",
      "[출력 구조 — 반드시 이 순서, 이 형식만, 섹션 사이 빈 줄 1개]",
      "",
      "[① 리스크 판정]",
      "첫 문장: '⚠️ 리스크 XX% — [한 줄 결론]'",
      ...
      "예시: 오늘 — 금품 반환 + 반환확인서 보관 / 3일 내 — 행동강령책임관 신고 / 1주일 — 권익위 1398 상담",
      "",
      "[⑤ 연결 질문]",
      "이 상황과 연결된 후속 질문 2개.",
    ].join("\n");
    ...
    const user = [
      ...
      "",
      "요구사항:",
      "1) [① 리스크 판정] 첫 문장은 '⚠️ 리스크 XX%' 형식으로 시작.",
      "2) [③ 처분 사례] 는 기관명/연도/처분결과/핵심사유 형식으로 1~2건 반드시 출력. '판례 없음' 절대 금지.",
      "3) [④ 즉시 조치] 는 행동 동사 시작 + 기한 포함 3단계.",
      "4) [⑤ 연결 질문] 은 이 상황과 연결된 질문 2개.",
      "5) 위 5개 섹션 외 다른 섹션 출력 금지.",
      "6) 판례 요지는 완결 문장으로 쓸 것.",
    ].join("\n");
    ...
    // [⑤ 연결 질문] 섹션에서 후속 질문 파싱
    const followUpSection = txt.match(/\[⑤ 연결 질문\]([\s\S]*?)$/)?.[1] ?? "";
