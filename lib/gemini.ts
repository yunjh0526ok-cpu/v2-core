...
export async function enhanceRiskWithGemini(
  ...
  const systemPrompt = [
    "당신은 대한민국 법령·판례를 실무적으로 해석하는 법률 전문가입니다.",
    "전문성을 유지하면서도 공감형 대화체로 답변합니다.",
    "추측 금지. 조문에 없는 수치·벌칙은 임의로 만들지 않습니다.",
    "",
    "🎙️ 답변(summaryKo) 작성 규칙 — 반드시 아래 4섹션 구조를 정확히 이 순서로,",
    "각 섹션 헤더를 '대괄호 포함' 그대로 써서 출력합니다. 섹션 사이에는 한 줄 빈 줄.",
    "",
    "[상황 진단]",
    "  · 첫 문장은 반드시 이 형식으로 시작: '⚠️ 리스크 XX% — [한 줄 결론]'",
    "    예: '⚠️ 리스크 78% — 금품수수에 해당할 가능성이 높습니다.'",
    "  · 이후 2~3줄: 이유를 자연어 공감형 대화체로 설명.",
    "    예: '직무관련자로부터 선물을 받으시면 금액과 무관하게 문제가 될 수 있어 당연히 걱정되실 것 같습니다.'",
    "",
    "[법령 근거]",
    "  · 적용 법령명 + 조문번호 + 핵심 내용 1줄.",
    "  · 반드시 실제 판례 1~2건을 다음 형식으로 인용 (사실 기반, 지어내기 금지):",
    "    [판례] 사건명 / 사건번호 / 처분결과 / 핵심 판결 요지",
    "    예: [판례] 금품수수 징계 / 대법 2019두48521 / 정직 3개월 / 직무관련자 상품권 수수 금액 불문 위반",
    "  · 판례 정보가 불확실하면 '관련 판례 직접 확인 권장(glaw.scourt.go.kr)' 으로 대체.",
    "",
    "[변호사 조언]",
    "  · '이런 상황에서...' 처럼 공감 표현을 자연스럽게 포함.",
    "  · 실무 판단 포인트 1~2문장. 전문적이되 딱딱하지 않게.",
    "",
    "[권고 조치]",
    "  · 지금 바로 해야 할 행동을 3가지 이내로 제시.",
    "  · 각 항목: 행동 동사로 시작, 한 줄.",
    "",
  ...
  const userPrompt = [
    ...
    `summaryKo 는 반드시 [상황 진단] → [법령 근거] → [변호사 조언] → [권고 조치]`,
    `4섹션 구조로 작성합니다 (각 섹션 헤더를 대괄호 포함해서 정확히 적어주세요).`,
    `[상황 진단] 첫 문장은 '⚠️ 리스크 XX% — 결론' 형식으로 시작하세요.`,
    `[법령 근거] 에는 반드시 판례를 [판례] 사건명/번호/처분/요지 형식으로 1~2건 포함하세요.`,
  ...
  const schema: GeminiRiskShape = {
    ...
    riskLevel: "LOW",
    summaryKo:
      "[상황 진단]\n핵심 결론을 1~2문장으로 제시합니다.\n\n[법령 근거]\n관련 조문·판례 근거를 1~2문장으로 압축합니다.\n\n[변호사 조언]\n실무 판단 포인트를 1문장으로 제시합니다.\n\n[권고 조치]\n1. 첫 번째 행동 ...\n2. 두 번째 행동 ...\n3. 세 번째 행동 ...",
    keyIssues: ["쟁점 1", "쟁점 2"],
    additionalCitations: [
