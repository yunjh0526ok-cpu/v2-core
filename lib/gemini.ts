...
export async function enhanceRiskWithGemini(
  ...
  const systemPrompt = [
    "당신은 대한민국 공공부문 청렴·윤리 분야에서 20년 동안 공무원을 교육해 온",
    "'친절한 청렴 전문 강사'입니다. 강의장에서 눈을 마주치며 설명하듯,",
    "따뜻하지만 정확한 톤으로 답변합니다.",
    "",
    "역할:",
    "1) 사용자 상황을 청탁금지법·이해충돌방지법·공직자윤리법·공무원 행동강령 등",
    "   현행법 체계 안에서 해석합니다.",
    "2) 아래 '규칙 엔진 결과' 와 '관련 조문 원문' 을 반드시 근거로 삼습니다.",
    "3) 추측 금지. 조문에 없는 수치/벌칙은 임의로 만들지 않습니다.",
    "",
    "🎙️ 답변(summaryKo) 작성 규칙 — 반드시 아래 4섹션 구조를 정확히 이 순서로,",
    "각 섹션 헤더를 '대괄호 포함' 그대로 써서 출력합니다. 섹션 사이에는 한 줄 빈 줄.",
    "",
    "[상황 진단]",
    "  · 입력된 상황의 법적·윤리적 쟁점이 무엇인지 2~3문장으로 친절히 설명.",
    "  · 공무원 입장에서 무엇이 문제인지, 왜 조심해야 하는지 이해하기 쉽게.",
    "",
    "[법령 근거]",
    "  · 관련 법 조항(청탁금지법 제8조 등)을 딱딱하지 않게 풀어서 2~3문장.",
    "  · '이 조항은 쉽게 말해…' 같은 강사 어투를 활용.",
    "",
    "[강사님의 한 줄 조언]",
    "  · 실무에서 가장 자주 놓치는 핵심 포인트 1~2문장.",
    "  · '제가 20년 동안 본 패턴으로는…' 처럼 경험담 섞어도 좋음.",
    "",
    "[권고 조치]",
    "  · 지금 바로 해야 할 행동을 '1. / 2. / 3.' 번호 매겨 3~5단계로 제시.",
    "  · 각 단계는 한 줄, 행동 동사로 시작 (예: '즉시 반환하세요' '서면으로 남기세요').",
    "",
    "문체: 존댓말, 따뜻한 강사 어투. '∼합니다 / ∼하세요 / ∼드립니다' 기조.",
    "추가로 summaryKo 는 최소 400자 이상으로 충분히 상세하게 작성합니다.",
  ...
  const userPrompt = [
    ...
    `summaryKo 는 반드시 [상황 진단] → [법령 근거] → [강사님의 한 줄 조언] → [권고 조치]`,
    `4섹션 구조로 작성합니다 (각 섹션 헤더를 대괄호 포함해서 정확히 적어주세요).`,
    `keyIssues 는 이 상황의 법적 쟁점을 2~4개 bullet 로 뽑아주세요.`,
    `recommendations 는 우선순위 순서로 3~5개 (짧은 행동 동사형 문장).`,
    `followUpQuestions 는 사용자가 다음 턴에 물어보면 좋을 후속 질문 2~3개.`,
  ].join("\n");
  ...
  const schema: GeminiRiskShape = {
    ...
    riskLevel: "LOW",
    summaryKo:
      "[상황 진단]\n이 상황에서는 ... (2~3문장)\n\n[법령 근거]\n관련 법령을 쉽게 말씀드리면 ... (2~3문장)\n\n[강사님의 한 줄 조언]\n제가 20년 동안 본 패턴으로는 ... (1~2문장)\n\n[권고 조치]\n1. 첫 번째 행동 ...\n2. 두 번째 행동 ...\n3. 세 번째 행동 ...",
    keyIssues: ["쟁점 1", "쟁점 2"],
    additionalCitations: [
...
function toRulesOnly(base: RiskAnalysis): EnhancedRiskAnalysis {
  ...
  const narrative = [
    `[상황 진단]`,
    `이 상황은 규칙 엔진 분석 기준 ${base.riskScore}% (${base.riskLevel}) 리스크로 판정되었습니다.` +
      ` 요약하자면, ${base.summary.split("\n")[0]}`,
    ``,
    `[법령 근거]`,
    top
      ? `가장 먼저 적용될 조항은 ${top.statute} ${top.clause} 입니다. ` +
        `쉽게 말해, 이 조항은 공직자가 직무와 관련해 취해야 할 기본 행동 원칙을 정해놓은 규정입니다.`
      : `직접 인용 조항을 추출하지 못했습니다. 유사 사례는 청탁금지법·이해충돌방지법을 1차 기준으로 검토합니다.`,
    ``,
    `[강사님의 한 줄 조언]`,
    `제가 20년 동안 본 패턴으로는, 이런 상황에서 머뭇거리는 시간이 길수록 리스크는 눈덩이처럼 커집니다. 판단이 애매할 때는 '거절' 과 '서면 기록' 이 가장 저렴한 보험입니다.`,
    ``,
    `[권고 조치]`,
    ...base.recommendations
      .slice(0, 5)
...
function buildFallbackDrama(input: DramatizeInput): DramatizeOutput {
