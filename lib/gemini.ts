...
export async function enhanceRiskWithGemini(
  ...
  const systemPrompt = [
    ...
    "",
    "summaryKo 는 반드시 아래 5섹션 구조만 사용. 섹션 사이 빈 줄 1개. 다른 섹션 추가 금지.",
    "",
    "[VERDICT]",
    "첫 줄: ✅ 또는 ❌ + 한 줄 판정문 (10자 이내, 직설적으로)",
    "둘째 줄: 리스크 XX% — 적용 법령 조문명",
    "예: ❌ 금품수수에 해당합니다",
    "    리스크 82% — 청탁금지법 제8조 제1항",
    "",
    "[WHY]",
    "3줄 이내 자연어 설명. 전문용어 최소화. 상황에 직접 대입해서 설명.",
    "예: '직무관련자가 준 선물은 금액에 관계없이 원칙적으로 받을 수 없습니다. 5만원 기준은 직무무관자에게만 적용됩니다.'",
    "",
    "[CASE]",
  ...
  const userPrompt = [
    ...
    `summaryKo 는 반드시 [VERDICT] → [WHY] → [CASE] → [ACTION] → [NEXT] 5섹션 구조로 작성.`,
    `[VERDICT] 첫 줄: ✅ 또는 ❌ + 판정문. 둘째 줄: 리스크 XX% — 법령명.`,
    `[CASE] 는 기관명/연도/처분결과/사건번호 1줄. 절대 생략 금지.`,
    `[ACTION] 은 ①②③ 번호형 즉시조치. 추상적 항목(청렴교육, 로드맵 등) 절대 금지.`,
